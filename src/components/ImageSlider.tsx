...
import { fetchAllSlides } from '../api/slides';
...
const ImageSlider: React.FC = () => {
  ...
  // fetch slide data on mount
  useEffect(() => {
    (async () => {
      try {
        const all = await fetchAllSlides();
        setSlides(all);
      } catch (err) {
        console.error(err);
      }
      ...
      {slides.map((s, i) => (
        ...
        >
          <img
            ...
            className="w-full h-full object-cover object-center scale-110 animate-kenburns"
          />
